...
const Profile = () => {
    ...
    return (
        <>
            ...
            <h1 className="flex mt-8 mb-8 uppercase text-xl tracking-wide items-start">
                Blogs
            </h1>
            <div className="w-full border rounded-sm shadow-sm border-neutral-700">
                <div>
                    <div className="p-4 rounded-sm md:p-8 bg-neutral-800">
                        <ol>
                            {sortedDates.map((date) => (
                                <li key={date} className="mb-6">
                                {groupedByDate[date].map((item, idx) => (
                                    <div key={1}>
                                        <h2 className="mb-3 text-3xl font-extrabold tracking-tight text-white">{item.title}</h2>
                                        <p className="mb-3 text-neutral-400">{item.summary}</p>
                                        <NavLink to='/blogs'>
                                            <div href="#" className="inline-flex items-center font-medium text-neutral-500 hover:text-neutral-700">
                                                View More
