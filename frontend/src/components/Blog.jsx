...
export const Blog = () => {
  ...
  if (selectedBlog) {
    return <BlogView blog={selectedBlog} onBack={() => setSelectedBlog(null)} />
  }
  ...
  return (
    <>
      <h1 className="flex justify-center mb-12 text-4xl font-extrabold leading-none tracking-tight md:text-5xl lg:text-6xl text-white">
        Blog Posts
      </h1>
          ...
          <li key={date} className="ms-4 mb-6">
            ...
            <ol className="mt-3 divide-y divide-gray-200 dark:divide-gray-700">
              {groupedByDate[date].map((item, idx) => (
                <li key={idx}>
                  <button
                      onClick={() => setSelectedBlog(item.metatitle)}
